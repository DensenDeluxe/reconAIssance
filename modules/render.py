...
def scriptmind_section(path):
    ...
    if os.path.exists(cve):
        ...
        html.append("<p><b>Linked CVEs (from script code):</b></p><ul>")
        html += [f"<li><a href='https://www.exploit-db.com/search?q={c}' target='_blank'>{c}</a></li>" for c in matches]
        html.append("</ul>")
    infer = os.path.join(path, "superscript_cve_infer.json")
    if os.path.exists(infer):
        inferred = json.load(open(infer))
        html.append("<p><b>Inferred CVEs (from behavior):</b></p><ul>")
        html += [f"<li><a href='https://nvd.nist.gov/vuln/detail/{c}' target='_blank'>{c}</a></li>" for c in inferred]
        html.append("</ul>")
...
def render(cves, path):
    html = [f"""<html><head><meta charset='utf-8'><title>Report</title>
    <link href='https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css' rel='stylesheet'></head>
    <body><div class='container mt-4'>
    <h1>ReconAIssance Report</h1><p><b>Timestamp:</b> {datetime.now()}</p>
    <h2>CVEs</h2>
    <table class='table table-bordered'><thead><tr><th>ID</th><th>Score</th><th>Severity</th><th>ExploitDB</th><th>AI</th></tr></thead><tbody>"""]
