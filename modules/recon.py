...
from load_api_keys import load_keys  # NEU
...
exit""")
    ...
    # GitHub leak search (neu mit API-Key zentral)
    gh_query = f"{target} password OR secret OR key OR token"
    headers = {'Accept': 'application/vnd.github.v3+json'}
    token = load_keys().get("github")
    if token:
        headers["Authorization"] = f"token {token}"
        logger.debug("Using GitHub token from api_keys.txt")
    else:
        logger.warning("No GitHub API token found – proceeding unauthenticated")
